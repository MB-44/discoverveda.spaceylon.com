...
function updateFloatingUIForBreathing(isActive) {
  ...
  const scrollLabel = document.querySelector('.floating-ui-scroll .floating-ui-label');
  if (scrollLabel) {
    scrollLabel.textContent = isActive ? 'SKIP' : 'SCROLL';
  }
}
...
function startBreathingExercise() {
  ...
  // Update floating UI to show SKIP
  updateFloatingUIForBreathing(true);
...
function startBreathingCycle() {
  ...
  function updateBreathingState(newState) {
    // Define text colors based on breathing state
    ...
    switch(newState) {
      case 'inhale':
        ...
        break;
      case 'hold':
        textColor = 'rgba(144, 238, 144, 1)'; // Particle color
        textShadow = '0 0 15px rgba(144, 238, 144, 0.4)';
        break;
      case 'exhale':
        textColor = 'rgba(100, 180, 100, 1)'; // Darker shade of particle color
        textShadow = '0 0 15px rgba(100, 180, 100, 0.4)';
        break;
      default:
        textColor = '#ffffff';
        textShadow = '0 0 15px rgba(255, 255, 255, 0.4)';
    ...
    // Update both breathing text and instruction colors
    const breathingText = document.querySelector('.breathing-text');
    
    gsap.to(breathingText, {
      color: textColor,
...
function stopBreathingExercise() {
  ...
  // Update floating UI back to SCROLL
  updateFloatingUIForBreathing(false);
...
window.addEventListener('load', () => {
  ...
  if (!history.state || !history.state.page) {
    history.pushState({page: 'scene3', timestamp: Date.now()}, '', '');
  }
});
